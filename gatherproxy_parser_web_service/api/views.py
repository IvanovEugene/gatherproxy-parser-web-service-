...
class CollectV1Handler:
    def __init__(self, app_config: dict):
        self._async_proxy_collector = AsyncProxyCollector(
            driver_params=app_config["driver_params"],
            validator_params=app_config["validator_params"],
    ...
    @docs(summary="Endpoint for collecting and returning in response list of "
                  "proxies by given parameters")
    ...
    async def collect(self, request: web.Request) -> web.Response:
        ...
        self._logger.info(f"New request {self._collect_request_id}",
                          extra={"request_id": request_id})
        try:
            form = CollectRequestSchema().loads(post_data)
        except Exception as validation_exception:
            self._logger.warning(f"Incorrect request body. "
                                 f"{validation_exception}",
                                 extra={"request_id": request_id})
            return web.json_response(data={"status": "Incorrect request body",
                                           "data": {},
                                           "errors": [str(validation_exception)]
                                           }, status=400)

        try:
            self._logger.info(f"Request body correct",
                              extra={"request_id": request_id})
            proxies = await self._async_proxy_collector.collect(
                page_count=form["page_count"], url_to_parse=form["url"],
            ...
            self._logger.error(f"Internal server error: {parse_exception}.",
                               extra={"request_id": request_id})
            return web.json_response(data={"status": "Internal server error",
                                           "data": {},
