...
class AsyncProxyCollector:
    def __init__(self, driver_params: dict, validator_params: dict, parser_max_workers: int):
    ...
    def _get_proxy_validator(self) -> ProxyValidator:
        validator_kwargs = {
            "proxy_verification_link": self._validator_params["verification_link"],
    ...
    async def collect(self, page_count: int, url_to_parse: str, request_id: int, validate: bool = True) -> set:
        # run parsing in ProcessPoolExecutor
        event_loop = get_event_loop()
        self._logger.debug("Start to collecting proxies", extra={"page_count": page_count, "url_to_parse": url_to_parse,
                                                                 "request_id": request_id})
        proxy_parser_worker = SyncProxyParserWorker(driver_params=self._driver_params)
        self._logger.debug("SyncProxyParserWorker created", extra={"request_id": request_id})
        asnyc_parse = partial(proxy_parser_worker.parse, page_count=page_count, url_to_parse=url_to_parse)
        proxies = await event_loop.run_in_executor(executor=self._executor, func=asnyc_parse)
        self._logger.info("Proxies collected", extra={"request_id": request_id})
        if not validate:
            return proxies
        self._logger.debug("Start to validating collected proxies", extra={"request_id": request_id})
